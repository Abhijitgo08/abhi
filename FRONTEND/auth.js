...
(() => {
  ...
  // Ensure location save finishes before redirect
    ...
    if (res.ok && data.token) {
      ...
      if (loc && Number.isFinite(loc.latitude) && Number.isFinite(loc.longitude)) {
        try {
          ...
          // wait until options are saved before redirect
          if (options.length > 0) {
      ...
      // only redirect AFTER location save finishes
      window.location.href = 'dashboard.html';
      return;
    } else {
